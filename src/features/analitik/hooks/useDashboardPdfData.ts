...
export const useDashboardPdfData = (periodLabel: string): DashboardPdfData => {
  ...
  const formattedData = useMemo<DashboardPdfData>(() => {
    ...
    const trendHariIniText = `${pTrendHariIni >= 0 ? "+" : ""}${pTrendHariIni.toFixed(1)}% vs rata-rata harian`;

    const avgMingguanBulanan = bulanIni / 4;
    const pTrendMingguIni = avgMingguanBulanan > 0 ? (mingguIni / avgMingguanBulanan * 100 - 100) : 0;
    const trendMingguIniText = `${pTrendMingguIni >= 0 ? "+" : ""}${pTrendMingguIni.toFixed(1)}% vs target omzet`;

    const rincianHarian = rawData.tabel_rincian_harian || [];
    const transaksiMapped = rincianHarian.map((item: any) => ({
    ...
    return {
      ...
      trendHarian: trendHariIniText,
      trendMingguan: trendMingguIniText,
      statusBulanan: "Status Periode Berjalan Aktif",
      periodLabel: periodLabel || "Dashboard Analitik",
      tanggalCetak: new Date().toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }),
  ...
  }, [response, productResponse, periodLabel]);
